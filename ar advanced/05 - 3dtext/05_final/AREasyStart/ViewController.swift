...
class ViewController: UIViewController {
    ...
    func updateMeasuringNodes() {
        ...
        if showMeasuring {
            ...
            for node in sceneView.scene.rootNode.childNodes {
                if node.name == "measuringline" || node.name == "measuretext" {
    ...
    func measure(fromNode: SCNNode, toNode: SCNNode) {
        ...
        let textGeometry = SCNText(string: "\(measurementValue) m", extrusionDepth: 1)
        textGeometry.flatness = 0.2
        let textNode = SCNNode(geometry: textGeometry)
        textNode.scale = SCNVector3Make(0.005, 0.005, 0.01)
        
        let billboardNode = SCNNode()
        billboardNode.name = "measuretext"
        billboardNode.position = toNode.position
        billboardNode.addChildNode(textNode)

        let billboardConstraints = SCNBillboardConstraint()
        billboardNode.constraints = [billboardConstraints]
        
        sceneView.scene.rootNode.addChildNode(billboardNode)
    }
...
extension ViewController : ARSCNViewDelegate {
    ...
    func renderer(_ renderer: SCNSceneRenderer, didUpdate node: SCNNode, for anchor: ARAnchor) {
        ...
        //don't draw the plane when measurement is active
        if measureButton.isSelected {
            self.clearAllDetectedPlanes()
            return
        }
